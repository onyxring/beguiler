# 15 Settings (`#beguilerSettings`)

<!-- toc -->
- [15.1 The Settings Block](#151-the-settings-block)
- [15.2 Toolchain Paths](#152-toolchain-paths)
- [15.3 Compilation Settings](#153-compilation-settings)
- [15.4 Runtime Settings](#154-runtime-settings)
- [15.5 Game Metadata](#155-game-metadata)
- [15.6 Blorb Packaging](#156-blorb-packaging)
  - [15.6.1 Asset Discovery and `_blorbAssets.bgl`](#1561-asset-discovery-and-blorbassetsbgl)
  - [15.6.2 IFID Generation and Persistence](#1562-ifid-generation-and-persistence)
- [15.7 Reading Settings From Source](#157-reading-settings-from-source)
- [15.8 Bindings and the Library Banner Constants](#158-bindings-and-the-library-banner-constants)
<!-- /toc -->

## 15.1 The Settings Block
...
permitted only at global scope (§12.7.1 lists the directive among the others). Any number of blocks
may appear, in the entry file or in any included file, and a block may be empty.

Each entry names one property from the fixed set in §15.2–§15.6 (Appendix E lists them all). The
...
command line counts as written before any block (§14.4). Three exceptions:
...
**See also** §14.4, §16.4, Appendix E.

## 15.2 Toolchain Paths
...
| `includePaths` | string | none | One or more directories, comma-separated, added to the search path for both `#include "…"` and `#includeI6` (§16.4). Additive across blocks and with `-includepaths=`. |

A regular `"…"` `includePaths` entry is normalized: separators are rewritten (§16.5), a relative path
...
| `outputPath` | string | `"output"` | Directory for the story file and every intermediate file; relative to the source file's directory (§18.1). |
...
| `omitUnusedRoutines` | bool | `true` | Emits `!% $OMIT_UNUSED_ROUTINES=1`, so Inform 6 drops routines nothing references (§16.11). |
| `economy` | bool | `false` | Opt-in automatic text abbreviation (§16.11). |

## 15.4 Runtime Settings
...
| Property | Type | Default | Description |
| --- | --- | --- | --- |
| `framePoolSize` | int | `64` | Word slots in the frame pool that backs Z-machine local-variable overflow and local arrays (§16.10). |
| `linqScratchSize` | int | `32` | Elements per scratch buffer for `<linq>` query chains (§20.5). Emitted only when `<linq>` is included. |
| `forInScratchSize` | int | `31` | Maximum elements in a literal-list `for (x in {…})` (§5.9.1). Emitted only when that form is used. |
| `rewritePaths` | bool | `true` | Rewrite `/` and `\` in every path (settings paths, `#include`, `#includeI6`) to the platform separator (§16.5). |
| `autoInitialize` | bool | `true` | When `true`, the library binding wraps the program's entry point so that `bglInit()` runs first (§19.2, §21.3.1). Set `false` when another library already replaces `Main`. |

The string pool used by `<string>` is not a setting: its size is the I6 constant
`bglStringPoolReserve`, declared in an `#i6` island before the extension is included (§20.3).

## 15.5 Game Metadata

These properties describe the work. They feed the iFiction record written into a blorb (§15.6) and
are readable from source (§15.7); apart from `release`, `serial` and `ifid` they are **not** emitted
...
| `ifid` | string | `""` | IFID in UUID form. Auto-generated when blorb packaging is on (§15.6.2). |
...
Everything else is available only through `#beguilerSettings.prop` (§15.7). The IF library bindings
use that mechanism to declare the library's `story` and `headline` constants from `title` and
`headline` (§21.3.2).

## 15.6 Blorb Packaging
...
When `generateBlorb` is `true`, two extra stages run (§16.2):
...
2. **After Inform 6 succeeds**, the story file and every discovered asset are assembled into a blorb
   ...
   (§18.2). The blorb carries an iFiction record built from §15.5.

A missing asset directory is reported and packaging proceeds with no assets.

### 15.6.1 Asset Discovery and `_blorbAssets.bgl`
...
union `eAssets = eImages | eSounds | eUnknownAsset` (§19.12). The generated `_blorbAssets.bgl` extends
each of them:
...
### 15.6.2 IFID Generation and Persistence
...
## 15.7 Reading Settings From Source
...
same properties are available as `##beguilerSettings.property` (§12.4.5).
...
**See also** §12.4.2, §12.4.5, §15.5.

## 15.8 Bindings and the Library Banner Constants

The IF library bindings declare the library's `story` and `headline` constants from the `title` and `headline` settings; see §21.3.2.
