...
- **Class-typed members with a constructor were never constructed.** A member whose type defines an `init` emitter — `object j { stringObj name = "fish"; }` — got the raw literal written into the property instead of a live instance, so reads printed garbage and method calls dispatched against the owning object. The same held for file-scope declarations. Locals were unaffected, which is why it went unnoticed. Both now run the type's `init` and apply the declared value through its `operator =`.
