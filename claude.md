...
It translates Beguile code (.bgl) into Inform 6 code (.inf), then passes the result to the I6 compiler to generate the final output (v5, v8, .ulx). 

On a successful compile it will do other things with the output from I6, so it waits for I6 to complete, rather than handing off produced I6 and ending. 
