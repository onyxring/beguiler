...
{   int j, k; uchar *p, c;
         ...
         j++)
    {
        ...
        {   for (k=2; p[k]!=0; k++)
            ...
            /* Glulx-only: do not overwrite matched text with 1s */
...
{   int32 i, j, k;
    ...
    /* Glulx-only: translate the text and place it into the Glulx string pool */
            ...
            if ((economy_switch) && (compression_switch) && (!is_abbreviation)
                ...
                if (text_in[i+1]=='@') {
                    ...
                    if (j == '@' || j == '\0') {
                        ...
                        if (j == 0) {
                            ...
                            if (!compression_switch)
                                warning("Ascii @@0 will prematurely terminate non-compressed string.");
                        }
                    }
                    write_z_char_g(j);
                    ...
                    if ((d1 == 127) || (d1 >= 10) || (d2 == 127) || (d2 >= 10)) {
                        ...
                        if (!compression_switch)
                            warning("'@..' print variable will not work in non-compressed string; substituting '   '.");
                        i += 2;
                        j = d1*10 + d2;
                        if (j >= MAX_DYNAMIC_STRINGS) {
                            error_max_dynamic_strings(j);
                    ...
                    if (unicode == '@' || unicode == '\0') {
                        ...
                        if (!compression_switch) {
                            warning("Unicode characters will not work in non-compressed string; substituting '?'.");
                            write_z_char_g('?');
                        }
                        else {
...
{
    /* Glulx-only: store inverted verb number in 16-bit field */
    int flag2 = (0xffff - infverb);
    if (dictword >= 0 && dictword < dict_entries)
    {
        uchar *p = dictionary + 4 + dictword*DICT_ENTRY_BYTE_LENGTH + DICT_ENTRY_FLAG_POS;
        p[2] = flag2 / 256; p[3] = flag2 % 256;
    }
}
