...
{   int i; char *name;
    ...
    /* Glulx-only: enforce Glulx attribute limit */
...
static void attributes_segment(void)
{
    ...
    {   truth_state = TRUE;
        ...
        /* Glulx-only: bits numbered from least to most significant. */
...
static void add_class_to_inheritance_list(int class_number)
{
    ...
    /* In Glulx, inherit attributes from the class into the Glulx attribute
       bytes (bits numbered LSB->MSB). */
    for (i=0; i<NUM_ATTR_BYTES; i++)
        full_object_g.atts[i]
            |= properties_table[class_info[class_number-1].begins_at - NUM_ATTR_BYTES + i];
}
...
{
    int i;
    /* Glulx-only: initialise the Glulx full_object structure */
...
inconvenience, please contact the maintainers.");
    ...
    /* Glulx: metaclasses must be backpatched, so mark used rather than system */
    ...
    /* Glulx: initial unnamed inheritance property (property 2) */
