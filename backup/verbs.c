...
{
    /* Glulx-only: only grammar version 2 is supported */
    if (val != 2) {
...
static void list_grammar_line_v2(int mark)
{
    ...
    /* Glulx-only: action uses two-byte base, then a flags byte */
    ...
    while (grammar_lines[mark] != 15) {
        ...
        /* Glulx-only: token data is a 32-bit big-endian value following the token byte */
