...
{   char *prefix_path;
    ...
    /* Glulx-only: always use the Glulx code extension */
...
{
    printf(
"\nThis program is a compiler of \"Glulx\" story files:\n Copyright (c) Graham Nelson 1993 - 2025.\n\n");
...
{   int i, s=1, state;
    ...
    {   state = TRUE;
        ...
        case 'v': s = select_glulx_version(p+i+1)+1; 
                  break;
...
{
    int len;
    snprintf(banner_line, CMD_BUF_SIZE, "Inform (Infrit seed) %d.%d%d",
