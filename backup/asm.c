...
{
    /* Glulx-only: choose the appropriate constant operand type */
    if (AO->value == 0)
...
{
    /* Glulx-only constant operand types */
...
{
    /* Glulx-only: variables are local or global */
...
extern char *variable_name(int32 i)
{
    ...
    /* Glulx-only variable names (globals start after locals) */
...
{
    /* Glulx-only: always use Glulx printer */
...
{   int i, rv;
    ...
    /* Glulx-only routine header assembly */
    rv = zmachine_pc;
...
{   int32 i;
    ...
    /* Glulx-only: transfer Glulx routine */
    transfer_routine_g();
    ...
    if (debugfile_switch)
    {
        ...
        {   debug_file_printf("<local-variable>");
            ...
            debug_file_printf("<frame-offset>%d</frame-offset>", 4 * (i - 1));
            debug_file_printf("</local-variable>");
        }
